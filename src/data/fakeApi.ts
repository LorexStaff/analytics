import { ProjectData } from "../entities/Project";

const fetchFakeApi = (): Promise<ProjectData[]> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const randomError = Math.random();

      if (randomError < 0.1) {
        reject({ statusCode: 404, message: "Данные не найдены." });
      } else if (randomError < 0.15) {
        reject({ statusCode: 403, message: "Доступ запрещён." });
      } else if (randomError < 0.2) {
        reject({
          statusCode: 401,
          message: "Неавторизованный доступ.",
        });
      } else if (randomError < 0.25) {
        reject({
          statusCode: 500,
          message: "Внутренняя ошибка сервера.",
        });
      } else if (randomError < 0.3) {
        reject({
          statusCode: 400,
          message: "Неверный запрос.",
        });
      } else if (randomError < 0.35) {
        reject({
          statusCode: 504,
          message: "Таймаут шлюза.",
        });
      } else if (randomError < 0.4) {
        reject({
          statusCode: 502,
          message: "Неверный шлюз.",
        });
      } else if (randomError < 0.45) {
        reject({
          statusCode: 304,
          message: "Ресурс не был изменён.",
        });
      } else if (randomError < 0.5) {
        reject({
          statusCode: 408,
          message: "Таймаут запроса.",
        });
      } else if (randomError < 0.55) {
        reject({
          statusCode: 503,
          message: "Сервис недоступен.",
        });
      } else {
        resolve(fakeApi);
      }
    }, 1000);
  });
};

export default fetchFakeApi;

const fakeApi: ProjectData[] = [
  {
    name: "Проект A",
    data: [
      {
        period: "01.04",
        newUsers: 50,
        activeUsers: 400,
        revenueGrowth: 1200,
        arpu: 30.5,
      },
      {
        period: "02.04",
        newUsers: 60,
        activeUsers: 420,
        revenueGrowth: 1250,
        arpu: 30.8,
      },
      {
        period: "03.04",
        newUsers: 45,
        activeUsers: 390,
        revenueGrowth: 1150,
        arpu: 30.0,
      },
      {
        period: "04.04",
        newUsers: 70,
        activeUsers: 450,
        revenueGrowth: 1300,
        arpu: 31.0,
      },
      {
        period: "05.04",
        newUsers: 55,
        activeUsers: 410,
        revenueGrowth: 1200,
        arpu: 30.3,
      },
      {
        period: "06.04",
        newUsers: 80,
        activeUsers: 470,
        revenueGrowth: 1350,
        arpu: 31.5,
      },
      {
        period: "07.04",
        newUsers: 65,
        activeUsers: 430,
        revenueGrowth: 1250,
        arpu: 30.8,
      },
      {
        period: "08.04",
        newUsers: 90,
        activeUsers: 500,
        revenueGrowth: 1400,
        arpu: 32.0,
      },
      {
        period: "09.04",
        newUsers: 75,
        activeUsers: 460,
        revenueGrowth: 1300,
        arpu: 31.2,
      },
      {
        period: "10.04",
        newUsers: 100,
        activeUsers: 520,
        revenueGrowth: 1450,
        arpu: 32.5,
      },
      {
        period: "11.04",
        newUsers: 85,
        activeUsers: 480,
        revenueGrowth: 1350,
        arpu: 31.8,
      },
      {
        period: "12.04",
        newUsers: 110,
        activeUsers: 540,
        revenueGrowth: 1500,
        arpu: 33.0,
      },
      {
        period: "13.04",
        newUsers: 95,
        activeUsers: 500,
        revenueGrowth: 1400,
        arpu: 32.2,
      },
      {
        period: "14.04",
        newUsers: 120,
        activeUsers: 560,
        revenueGrowth: 1550,
        arpu: 33.5,
      },
      {
        period: "15.04",
        newUsers: 105,
        activeUsers: 520,
        revenueGrowth: 1450,
        arpu: 32.8,
      },
      {
        period: "16.04",
        newUsers: 130,
        activeUsers: 580,
        revenueGrowth: 1600,
        arpu: 34.0,
      },
      {
        period: "17.04",
        newUsers: 115,
        activeUsers: 540,
        revenueGrowth: 1500,
        arpu: 33.2,
      },
      {
        period: "18.04",
        newUsers: 140,
        activeUsers: 600,
        revenueGrowth: 1650,
        arpu: 34.5,
      },
      {
        period: "19.04",
        newUsers: 125,
        activeUsers: 560,
        revenueGrowth: 1550,
        arpu: 33.8,
      },
      {
        period: "20.04",
        newUsers: 150,
        activeUsers: 620,
        revenueGrowth: 1700,
        arpu: 35.0,
      },
      {
        period: "21.04",
        newUsers: 135,
        activeUsers: 580,
        revenueGrowth: 1600,
        arpu: 34.2,
      },
      {
        period: "22.04",
        newUsers: 160,
        activeUsers: 640,
        revenueGrowth: 1750,
        arpu: 35.5,
      },
      {
        period: "23.04",
        newUsers: 145,
        activeUsers: 600,
        revenueGrowth: 1650,
        arpu: 34.8,
      },
      {
        period: "24.04",
        newUsers: 170,
        activeUsers: 660,
        revenueGrowth: 1800,
        arpu: 36.0,
      },
      {
        period: "25.04",
        newUsers: 155,
        activeUsers: 620,
        revenueGrowth: 1700,
        arpu: 35.2,
      },
      {
        period: "26.04",
        newUsers: 180,
        activeUsers: 680,
        revenueGrowth: 1850,
        arpu: 36.5,
      },
      {
        period: "27.04",
        newUsers: 165,
        activeUsers: 640,
        revenueGrowth: 1750,
        arpu: 35.8,
      },
      {
        period: "28.04",
        newUsers: 190,
        activeUsers: 700,
        revenueGrowth: 1900,
        arpu: 37.0,
      },
      {
        period: "29.04",
        newUsers: 175,
        activeUsers: 660,
        revenueGrowth: 1800,
        arpu: 36.2,
      },
      {
        period: "30.04",
        newUsers: 200,
        activeUsers: 720,
        revenueGrowth: 1950,
        arpu: 37.5,
      },
      {
        period: "01.05",
        newUsers: 150,
        activeUsers: 720,
        revenueGrowth: 1950,
        arpu: 37.8,
      },
      {
        period: "02.05",
        newUsers: 140,
        activeUsers: 700,
        revenueGrowth: 1850,
        arpu: 36.5,
      },
      {
        period: "03.05",
        newUsers: 160,
        activeUsers: 740,
        revenueGrowth: 2000,
        arpu: 38.0,
      },
      {
        period: "04.05",
        newUsers: 130,
        activeUsers: 680,
        revenueGrowth: 1750,
        arpu: 35.5,
      },
      {
        period: "05.05",
        newUsers: 170,
        activeUsers: 760,
        revenueGrowth: 2100,
        arpu: 39.0,
      },
      {
        period: "06.05",
        newUsers: 120,
        activeUsers: 660,
        revenueGrowth: 1700,
        arpu: 34.8,
      },
      {
        period: "07.05",
        newUsers: 180,
        activeUsers: 780,
        revenueGrowth: 2200,
        arpu: 40.0,
      },
      {
        period: "08.05",
        newUsers: 110,
        activeUsers: 640,
        revenueGrowth: 1650,
        arpu: 33.8,
      },
      {
        period: "09.05",
        newUsers: 190,
        activeUsers: 800,
        revenueGrowth: 2300,
        arpu: 41.0,
      },
      {
        period: "10.05",
        newUsers: 100,
        activeUsers: 620,
        revenueGrowth: 1600,
        arpu: 32.5,
      },
      {
        period: "11.05",
        newUsers: 200,
        activeUsers: 820,
        revenueGrowth: 2400,
        arpu: 42.0,
      },
      {
        period: "12.05",
        newUsers: 90,
        activeUsers: 600,
        revenueGrowth: 1550,
        arpu: 31.2,
      },
      {
        period: "13.05",
        newUsers: 210,
        activeUsers: 840,
        revenueGrowth: 2500,
        arpu: 43.0,
      },
      {
        period: "14.05",
        newUsers: 80,
        activeUsers: 580,
        revenueGrowth: 1500,
        arpu: 30.0,
      },
      {
        period: "15.05",
        newUsers: 220,
        activeUsers: 860,
        revenueGrowth: 2600,
        arpu: 44.0,
      },
      {
        period: "16.05",
        newUsers: 70,
        activeUsers: 560,
        revenueGrowth: 1450,
        arpu: 28.8,
      },
      {
        period: "17.05",
        newUsers: 230,
        activeUsers: 880,
        revenueGrowth: 2700,
        arpu: 45.0,
      },
      {
        period: "18.05",
        newUsers: 60,
        activeUsers: 540,
        revenueGrowth: 1400,
        arpu: 27.5,
      },
      {
        period: "19.05",
        newUsers: 240,
        activeUsers: 900,
        revenueGrowth: 2800,
        arpu: 46.0,
      },
      {
        period: "20.05",
        newUsers: 50,
        activeUsers: 520,
        revenueGrowth: 1350,
        arpu: 26.2,
      },
      {
        period: "21.05",
        newUsers: 250,
        activeUsers: 920,
        revenueGrowth: 2900,
        arpu: 47.0,
      },
      {
        period: "22.05",
        newUsers: 40,
        activeUsers: 500,
        revenueGrowth: 1300,
        arpu: 25.0,
      },
      {
        period: "23.05",
        newUsers: 260,
        activeUsers: 940,
        revenueGrowth: 3000,
        arpu: 48.0,
      },
      {
        period: "24.05",
        newUsers: 30,
        activeUsers: 480,
        revenueGrowth: 1250,
        arpu: 23.8,
      },
      {
        period: "25.05",
        newUsers: 270,
        activeUsers: 960,
        revenueGrowth: 3100,
        arpu: 49.0,
      },
      {
        period: "26.05",
        newUsers: 20,
        activeUsers: 460,
        revenueGrowth: 1200,
        arpu: 22.5,
      },
      {
        period: "27.05",
        newUsers: 280,
        activeUsers: 980,
        revenueGrowth: 3200,
        arpu: 50.0,
      },
      {
        period: "28.05",
        newUsers: 10,
        activeUsers: 440,
        revenueGrowth: 1150,
        arpu: 21.2,
      },
      {
        period: "29.05",
        newUsers: 290,
        activeUsers: 1000,
        revenueGrowth: 3300,
        arpu: 51.0,
      },
      {
        period: "30.05",
        newUsers: 5,
        activeUsers: 420,
        revenueGrowth: 1100,
        arpu: 20.0,
      },
      {
        period: "31.05",
        newUsers: 300,
        activeUsers: 1020,
        revenueGrowth: 3400,
        arpu: 52.0,
      },
      {
        period: "01.06",
        newUsers: 250,
        activeUsers: 1020,
        revenueGrowth: 3400,
        arpu: 52.0,
      },
      {
        period: "02.06",
        newUsers: 240,
        activeUsers: 1000,
        revenueGrowth: 3300,
        arpu: 51.0,
      },
      {
        period: "03.06",
        newUsers: 260,
        activeUsers: 1040,
        revenueGrowth: 3500,
        arpu: 53.0,
      },
      {
        period: "04.06",
        newUsers: 230,
        activeUsers: 980,
        revenueGrowth: 3200,
        arpu: 50.0,
      },
      {
        period: "05.06",
        newUsers: 270,
        activeUsers: 1060,
        revenueGrowth: 3600,
        arpu: 54.0,
      },
      {
        period: "06.06",
        newUsers: 220,
        activeUsers: 960,
        revenueGrowth: 3100,
        arpu: 49.0,
      },
      {
        period: "07.06",
        newUsers: 280,
        activeUsers: 1080,
        revenueGrowth: 3700,
        arpu: 55.0,
      },
      {
        period: "08.06",
        newUsers: 210,
        activeUsers: 940,
        revenueGrowth: 3000,
        arpu: 48.0,
      },
      {
        period: "09.06",
        newUsers: 290,
        activeUsers: 1100,
        revenueGrowth: 3800,
        arpu: 56.0,
      },
      {
        period: "10.06",
        newUsers: 200,
        activeUsers: 920,
        revenueGrowth: 2900,
        arpu: 47.0,
      },
      {
        period: "11.06",
        newUsers: 300,
        activeUsers: 1120,
        revenueGrowth: 3900,
        arpu: 57.0,
      },
      {
        period: "12.06",
        newUsers: 190,
        activeUsers: 900,
        revenueGrowth: 2800,
        arpu: 46.0,
      },
      {
        period: "13.06",
        newUsers: 310,
        activeUsers: 1140,
        revenueGrowth: 4000,
        arpu: 58.0,
      },
      {
        period: "14.06",
        newUsers: 180,
        activeUsers: 880,
        revenueGrowth: 2700,
        arpu: 45.0,
      },
      {
        period: "15.06",
        newUsers: 320,
        activeUsers: 1160,
        revenueGrowth: 4100,
        arpu: 59.0,
      },
      {
        period: "16.06",
        newUsers: 170,
        activeUsers: 860,
        revenueGrowth: 2600,
        arpu: 44.0,
      },
      {
        period: "17.06",
        newUsers: 330,
        activeUsers: 1180,
        revenueGrowth: 4200,
        arpu: 60.0,
      },
      {
        period: "18.06",
        newUsers: 160,
        activeUsers: 840,
        revenueGrowth: 2500,
        arpu: 43.0,
      },
      {
        period: "19.06",
        newUsers: 340,
        activeUsers: 1200,
        revenueGrowth: 4300,
        arpu: 61.0,
      },
      {
        period: "20.06",
        newUsers: 150,
        activeUsers: 820,
        revenueGrowth: 2400,
        arpu: 42.0,
      },
      {
        period: "21.06",
        newUsers: 350,
        activeUsers: 1220,
        revenueGrowth: 4400,
        arpu: 62.0,
      },
      {
        period: "22.06",
        newUsers: 140,
        activeUsers: 800,
        revenueGrowth: 2300,
        arpu: 41.0,
      },
      {
        period: "23.06",
        newUsers: 360,
        activeUsers: 1240,
        revenueGrowth: 4500,
        arpu: 63.0,
      },
      {
        period: "24.06",
        newUsers: 130,
        activeUsers: 780,
        revenueGrowth: 2200,
        arpu: 40.0,
      },
      {
        period: "25.06",
        newUsers: 370,
        activeUsers: 1260,
        revenueGrowth: 4600,
        arpu: 64.0,
      },
      {
        period: "26.06",
        newUsers: 120,
        activeUsers: 760,
        revenueGrowth: 2100,
        arpu: 39.0,
      },
      {
        period: "27.06",
        newUsers: 380,
        activeUsers: 1280,
        revenueGrowth: 4700,
        arpu: 65.0,
      },
      {
        period: "28.06",
        newUsers: 110,
        activeUsers: 740,
        revenueGrowth: 2000,
        arpu: 38.0,
      },
      {
        period: "29.06",
        newUsers: 390,
        activeUsers: 1300,
        revenueGrowth: 4800,
        arpu: 66.0,
      },
      {
        period: "30.06",
        newUsers: 100,
        activeUsers: 720,
        revenueGrowth: 1900,
        arpu: 37.0,
      },
    ],
  },
  {
    name: "Проект B",
    data: [
      {
        period: "01.04",
        newUsers: 30,
        activeUsers: 300,
        revenueGrowth: 1000,
        arpu: 25.0,
      },
      {
        period: "02.04",
        newUsers: 40,
        activeUsers: 320,
        revenueGrowth: 1050,
        arpu: 25.5,
      },
      {
        period: "03.04",
        newUsers: 25,
        activeUsers: 290,
        revenueGrowth: 950,
        arpu: 24.8,
      },
      {
        period: "04.04",
        newUsers: 50,
        activeUsers: 350,
        revenueGrowth: 1100,
        arpu: 26.0,
      },
      {
        period: "05.04",
        newUsers: 35,
        activeUsers: 310,
        revenueGrowth: 1000,
        arpu: 25.3,
      },
      {
        period: "06.04",
        newUsers: 60,
        activeUsers: 370,
        revenueGrowth: 1150,
        arpu: 26.5,
      },
      {
        period: "07.04",
        newUsers: 45,
        activeUsers: 330,
        revenueGrowth: 1050,
        arpu: 25.8,
      },
      {
        period: "08.04",
        newUsers: 70,
        activeUsers: 400,
        revenueGrowth: 1200,
        arpu: 27.0,
      },
      {
        period: "09.04",
        newUsers: 55,
        activeUsers: 360,
        revenueGrowth: 1100,
        arpu: 26.3,
      },
      {
        period: "10.04",
        newUsers: 80,
        activeUsers: 420,
        revenueGrowth: 1250,
        arpu: 27.5,
      },
      {
        period: "11.04",
        newUsers: 65,
        activeUsers: 380,
        revenueGrowth: 1150,
        arpu: 26.8,
      },
      {
        period: "12.04",
        newUsers: 90,
        activeUsers: 440,
        revenueGrowth: 1300,
        arpu: 28.0,
      },
      {
        period: "13.04",
        newUsers: 75,
        activeUsers: 400,
        revenueGrowth: 1200,
        arpu: 27.3,
      },
      {
        period: "14.04",
        newUsers: 100,
        activeUsers: 460,
        revenueGrowth: 1350,
        arpu: 28.5,
      },
      {
        period: "15.04",
        newUsers: 85,
        activeUsers: 420,
        revenueGrowth: 1250,
        arpu: 27.8,
      },
      {
        period: "16.04",
        newUsers: 110,
        activeUsers: 480,
        revenueGrowth: 1400,
        arpu: 29.0,
      },
      {
        period: "17.04",
        newUsers: 95,
        activeUsers: 440,
        revenueGrowth: 1300,
        arpu: 28.3,
      },
      {
        period: "18.04",
        newUsers: 120,
        activeUsers: 500,
        revenueGrowth: 1450,
        arpu: 29.5,
      },
      {
        period: "19.04",
        newUsers: 105,
        activeUsers: 460,
        revenueGrowth: 1350,
        arpu: 28.8,
      },
      {
        period: "20.04",
        newUsers: 130,
        activeUsers: 520,
        revenueGrowth: 1500,
        arpu: 30.0,
      },
      {
        period: "21.04",
        newUsers: 115,
        activeUsers: 480,
        revenueGrowth: 1400,
        arpu: 29.3,
      },
      {
        period: "22.04",
        newUsers: 140,
        activeUsers: 540,
        revenueGrowth: 1550,
        arpu: 30.5,
      },
      {
        period: "23.04",
        newUsers: 125,
        activeUsers: 500,
        revenueGrowth: 1450,
        arpu: 29.8,
      },
      {
        period: "24.04",
        newUsers: 150,
        activeUsers: 560,
        revenueGrowth: 1600,
        arpu: 31.0,
      },
      {
        period: "25.04",
        newUsers: 135,
        activeUsers: 520,
        revenueGrowth: 1500,
        arpu: 30.3,
      },
      {
        period: "26.04",
        newUsers: 160,
        activeUsers: 580,
        revenueGrowth: 1650,
        arpu: 31.5,
      },
      {
        period: "27.04",
        newUsers: 145,
        activeUsers: 540,
        revenueGrowth: 1550,
        arpu: 30.8,
      },
      {
        period: "28.04",
        newUsers: 170,
        activeUsers: 600,
        revenueGrowth: 1700,
        arpu: 32.0,
      },
      {
        period: "29.04",
        newUsers: 155,
        activeUsers: 560,
        revenueGrowth: 1600,
        arpu: 39.0,
      },
      {
        period: "30.04",
        newUsers: 175,
        activeUsers: 590,
        revenueGrowth: 2450,
        arpu: 39.5,
      },
      {
        period: "01.05",
        newUsers: 80,
        activeUsers: 590,
        revenueGrowth: 1600,
        arpu: 36.2,
      },
      {
        period: "02.05",
        newUsers: 70,
        activeUsers: 570,
        revenueGrowth: 1500,
        arpu: 34.5,
      },
      {
        period: "03.05",
        newUsers: 90,
        activeUsers: 610,
        revenueGrowth: 1700,
        arpu: 37.0,
      },
      {
        period: "04.05",
        newUsers: 60,
        activeUsers: 550,
        revenueGrowth: 1400,
        arpu: 32.8,
      },
      {
        period: "05.05",
        newUsers: 100,
        activeUsers: 630,
        revenueGrowth: 1800,
        arpu: 38.5,
      },
      {
        period: "06.05",
        newUsers: 50,
        activeUsers: 530,
        revenueGrowth: 1350,
        arpu: 31.2,
      },
      {
        period: "07.05",
        newUsers: 110,
        activeUsers: 650,
        revenueGrowth: 1900,
        arpu: 39.5,
      },
      {
        period: "08.05",
        newUsers: 40,
        activeUsers: 510,
        revenueGrowth: 1300,
        arpu: 30.0,
      },
      {
        period: "09.05",
        newUsers: 120,
        activeUsers: 670,
        revenueGrowth: 2000,
        arpu: 40.5,
      },
      {
        period: "10.05",
        newUsers: 30,
        activeUsers: 490,
        revenueGrowth: 1250,
        arpu: 28.5,
      },
      {
        period: "11.05",
        newUsers: 130,
        activeUsers: 690,
        revenueGrowth: 2100,
        arpu: 41.5,
      },
      {
        period: "12.05",
        newUsers: 20,
        activeUsers: 470,
        revenueGrowth: 1200,
        arpu: 26.8,
      },
      {
        period: "13.05",
        newUsers: 140,
        activeUsers: 710,
        revenueGrowth: 2200,
        arpu: 42.5,
      },
      {
        period: "14.05",
        newUsers: 10,
        activeUsers: 450,
        revenueGrowth: 1150,
        arpu: 25.0,
      },
      {
        period: "15.05",
        newUsers: 150,
        activeUsers: 730,
        revenueGrowth: 2300,
        arpu: 43.5,
      },
      {
        period: "16.05",
        newUsers: 5,
        activeUsers: 430,
        revenueGrowth: 1100,
        arpu: 23.5,
      },
      {
        period: "17.05",
        newUsers: 160,
        activeUsers: 750,
        revenueGrowth: 2400,
        arpu: 44.5,
      },
      {
        period: "18.05",
        newUsers: 0,
        activeUsers: 410,
        revenueGrowth: 1050,
        arpu: 22.0,
      },
      {
        period: "19.05",
        newUsers: 170,
        activeUsers: 770,
        revenueGrowth: 2500,
        arpu: 45.5,
      },
      {
        period: "20.05",
        newUsers: 0,
        activeUsers: 390,
        revenueGrowth: 1000,
        arpu: 20.5,
      },
      {
        period: "21.05",
        newUsers: 180,
        activeUsers: 790,
        revenueGrowth: 2600,
        arpu: 46.5,
      },
      {
        period: "22.05",
        newUsers: 0,
        activeUsers: 370,
        revenueGrowth: 950,
        arpu: 19.0,
      },
      {
        period: "23.05",
        newUsers: 190,
        activeUsers: 810,
        revenueGrowth: 2700,
        arpu: 47.5,
      },
      {
        period: "24.05",
        newUsers: 0,
        activeUsers: 350,
        revenueGrowth: 900,
        arpu: 17.5,
      },
      {
        period: "25.05",
        newUsers: 200,
        activeUsers: 830,
        revenueGrowth: 2800,
        arpu: 48.5,
      },
      {
        period: "26.05",
        newUsers: 0,
        activeUsers: 330,
        revenueGrowth: 850,
        arpu: 16.0,
      },
      {
        period: "27.05",
        newUsers: 210,
        activeUsers: 850,
        revenueGrowth: 2900,
        arpu: 49.5,
      },
      {
        period: "28.05",
        newUsers: 0,
        activeUsers: 310,
        revenueGrowth: 800,
        arpu: 14.5,
      },
      {
        period: "29.05",
        newUsers: 220,
        activeUsers: 870,
        revenueGrowth: 3000,
        arpu: 50.5,
      },
      {
        period: "30.05",
        newUsers: 0,
        activeUsers: 290,
        revenueGrowth: 750,
        arpu: 13.0,
      },
      {
        period: "31.05",
        newUsers: 230,
        activeUsers: 890,
        revenueGrowth: 3100,
        arpu: 51.5,
      },
      {
        period: "01.06",
        newUsers: 150,
        activeUsers: 590,
        revenueGrowth: 1600,
        arpu: 36.2,
      },
      {
        period: "02.06",
        newUsers: 140,
        activeUsers: 570,
        revenueGrowth: 1500,
        arpu: 34.5,
      },
      {
        period: "03.06",
        newUsers: 160,
        activeUsers: 610,
        revenueGrowth: 1700,
        arpu: 37.0,
      },
      {
        period: "04.06",
        newUsers: 130,
        activeUsers: 550,
        revenueGrowth: 1400,
        arpu: 32.8,
      },
      {
        period: "05.06",
        newUsers: 170,
        activeUsers: 630,
        revenueGrowth: 1800,
        arpu: 38.5,
      },
      {
        period: "06.06",
        newUsers: 120,
        activeUsers: 530,
        revenueGrowth: 1350,
        arpu: 31.2,
      },
      {
        period: "07.06",
        newUsers: 180,
        activeUsers: 650,
        revenueGrowth: 1900,
        arpu: 39.5,
      },
      {
        period: "08.06",
        newUsers: 110,
        activeUsers: 510,
        revenueGrowth: 1300,
        arpu: 30.0,
      },
      {
        period: "09.06",
        newUsers: 190,
        activeUsers: 670,
        revenueGrowth: 2000,
        arpu: 40.5,
      },
      {
        period: "10.06",
        newUsers: 100,
        activeUsers: 490,
        revenueGrowth: 1250,
        arpu: 28.5,
      },
      {
        period: "11.06",
        newUsers: 200,
        activeUsers: 690,
        revenueGrowth: 2100,
        arpu: 41.5,
      },
      {
        period: "12.06",
        newUsers: 90,
        activeUsers: 470,
        revenueGrowth: 1200,
        arpu: 26.8,
      },
      {
        period: "13.06",
        newUsers: 210,
        activeUsers: 710,
        revenueGrowth: 2200,
        arpu: 42.5,
      },
      {
        period: "14.06",
        newUsers: 80,
        activeUsers: 450,
        revenueGrowth: 1150,
        arpu: 25.0,
      },
      {
        period: "15.06",
        newUsers: 220,
        activeUsers: 730,
        revenueGrowth: 2300,
        arpu: 43.5,
      },
      {
        period: "16.06",
        newUsers: 70,
        activeUsers: 430,
        revenueGrowth: 1100,
        arpu: 23.5,
      },
      {
        period: "17.06",
        newUsers: 230,
        activeUsers: 750,
        revenueGrowth: 2400,
        arpu: 44.5,
      },
      {
        period: "18.06",
        newUsers: 60,
        activeUsers: 410,
        revenueGrowth: 1050,
        arpu: 22.0,
      },
      {
        period: "19.06",
        newUsers: 240,
        activeUsers: 770,
        revenueGrowth: 2500,
        arpu: 45.5,
      },
      {
        period: "20.06",
        newUsers: 50,
        activeUsers: 390,
        revenueGrowth: 1000,
        arpu: 20.5,
      },
      {
        period: "21.06",
        newUsers: 250,
        activeUsers: 790,
        revenueGrowth: 2600,
        arpu: 46.5,
      },
      {
        period: "22.06",
        newUsers: 40,
        activeUsers: 370,
        revenueGrowth: 950,
        arpu: 19.0,
      },
      {
        period: "23.06",
        newUsers: 260,
        activeUsers: 810,
        revenueGrowth: 2700,
        arpu: 47.5,
      },
      {
        period: "24.06",
        newUsers: 30,
        activeUsers: 350,
        revenueGrowth: 900,
        arpu: 17.5,
      },
      {
        period: "25.06",
        newUsers: 270,
        activeUsers: 830,
        revenueGrowth: 2800,
        arpu: 48.5,
      },
      {
        period: "26.06",
        newUsers: 20,
        activeUsers: 330,
        revenueGrowth: 850,
        arpu: 16.0,
      },
      {
        period: "27.06",
        newUsers: 280,
        activeUsers: 850,
        revenueGrowth: 2900,
        arpu: 49.5,
      },
      {
        period: "28.06",
        newUsers: 10,
        activeUsers: 310,
        revenueGrowth: 800,
        arpu: 14.5,
      },
      {
        period: "29.06",
        newUsers: 290,
        activeUsers: 870,
        revenueGrowth: 3000,
        arpu: 50.5,
      },
      {
        period: "30.06",
        newUsers: 5,
        activeUsers: 290,
        revenueGrowth: 750,
        arpu: 13.0,
      },
    ],
  },
];
